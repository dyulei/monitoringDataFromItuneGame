...
def get_data():
    ...
    tb_rank_orderby_releasetime = ("select * from db_rankapp.tb_rank t where t.created_at >= '2014-08-01' and t.created_at <= '2014-08-07' and t.rank_type = 1 order by app_id")
    ...
    startDate = datetime.datetime.strptime('2014-08-01', '%Y-%m-%d')
    ...
    for rank in recs:
        app_id = rank[1]
        # created_at = str(rank[4])
        created_at = datetime.datetime.strptime(str(rank[4])[0:10], '%Y-%m-%d')
        rank_num = rank[3]
